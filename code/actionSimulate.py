...
def backpropagateRewardsTDSimulate(mdp, td, numTrials=10, maxIterations=50, verbose=False,
             sort=False):
    ...
    for trial in range(numTrials):
        ...
        allStates = [] #keep track of all states so that we can backpropagate end reward

        for _ in range(maxIterations):
            allStates.append(state) 
            actions = mdp.actions(state)
            #if it's an end state, the only action will be idle.
            #TODO: should be replacable by...if is end state.. right?
            if actions == ["idle"]:
                break
            ...
            if mdp.endOfGame(newState):
                ...
                td.backpropagateReward(reward, allStates)
            else:
